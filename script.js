(function () {
  ...
  // --- iOS Waitlist Form Toggle ---
  var iosToggle = document.getElementById('iosWaitlistToggle');
  var iosEmbed = document.getElementById('iosFormEmbed');
  if (iosToggle && iosEmbed) {
    iosToggle.addEventListener('click', function () {
      var open = iosEmbed.style.display !== 'none';
      iosEmbed.style.display = open ? 'none' : 'block';
      iosToggle.textContent = open ? 'Join iOS Waitlist' : 'Hide Form';
    });
  }
  ...
  if (appBanner && bannerClose) {
    ...
    if (!bannerDismissed) {
      setTimeout(function () { appBanner.classList.add('visible'); }, 3000);
    }
